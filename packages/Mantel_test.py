...
def distance_matrix(X, num_of_sample):
    '''
    for mantel test
    get distance matrix
    '''
    centroid = [[] for i in range(4)]
    for i in range(4):
        cur = X[num_of_sample*i*3:num_of_sample*(i+1)*3, :]
        for feat in range(cur.shape[1]):
            ave = np.mean(cur[:, feat])
            centroid[i].append(ave)
    centroid = np.array(centroid)
    distances = np.array([np.linalg.norm(centroid[i] - centroid[j]) for i in range(4) for j in range(i+1, 4)])
    return distances
